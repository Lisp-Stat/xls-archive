(defun erfc (x) (declare (type double-float x))
 (prog
  ((w 0.0d0) (a (make-array '(4) :element-type 'float))
   ...
   (s (make-array '(5) :element-type 'float)) (erfc 0.0d0) (c 0.0d0) (e 0.0d0)
  ...
  (declare (type (simple-array float (*)) s)) (declare (type real erfc))
  ...
  (setf erfc (+ 0.5 (+ 0.5 (/ (* (* -1 x) top) bot)))) (go end_label) label10
  ...
  (setf erfc (/ top bot)) label11 (setf w (* (dble x) (dble x))) (setf t_ w)
  (setf e (+ w (- (dble t_))))
  (setf erfc (* (* (+ 0.5 (+ 0.5 (- e))) (exp (- t_))) erfc))
  (if (< x 0.0) (setf erfc (+ 2.0 (- erfc)))) (go end_label) label20
  (if (<= x (- 5.5)) (go label30)) (if (> x 50.0) (go label40))
  ...
  (setf erfc (/ (+ c (/ (* (* -1 t_) top) bot)) ax)) (go label11) label30
  (setf erfc 2.0) (go end_label) label40 (setf erfc 0.0) (go end_label)
  end_label (return erfc)
))
