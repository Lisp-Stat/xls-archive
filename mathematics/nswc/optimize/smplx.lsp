(defun smplx (a b0 c ka m n0 ind ibasis x z iter mxiter numle numge bi wk iwk)
 (declare (type (simple-array double-float (* *)) a))
 (declare (type (simple-array double-float (*)) b0))
 (declare (type (simple-array double-float (*)) c)) (declare (type fixnum ka))
 (declare (type fixnum m)) (declare (type fixnum n0))
 (declare (type fixnum ind)) (declare (type (simple-array fixnum (*)) ibasis))
 ...
 (prog ((ip 0) (rerrmx 0.0d0) (1.0e%6 0.0d0) (rerrmn 0.0d0) (eps0 0.0d0))
  ...
  (multiple-value-setq
   (a b0 c ka m n0 ind ibasis x z iter mxiter eps0 rerrmn rerrmx rerr numle
    numge bi dummy_var dummy_var dummy_var dummy_var
   )
   (smplx1 a b0 c ka m n0 ind ibasis x z iter mxiter eps0 rerrmn rerrmx rerr
    numle numge bi (fref wk 1) (fref wk (+ m 1)) (fref iwk 1) (fref iwk ip)
  ))
  (return
   (values a b0 c ka m n0 ind ibasis x z iter mxiter numle numge bi wk iwk)
)))
