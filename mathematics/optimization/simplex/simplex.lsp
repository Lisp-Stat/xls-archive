(defmacro vector-push (n a fp)
          `(cond ((>= ,fp (array-dimension ,a 0))
                  nil)
                 (t
                  (setf (aref ,a ,fp) ,n)
                  (incf ,fp)
...
(deftype Zahl () 'float) ; abbreviation: R
...
(defun simplex (A b c)
  ...
  (let ((m (length b))
    ...
    (let ((AA (let ((h (make-array `(,m ,n))))
                (dotimes (i m)
                  (dotimes (j n)
                    (setf (aref h i j) (aref A i j))
                ) )
                h
          )   )
          (AB (let ((h (make-array m)))
                (dotimes (i m) (setf (aref h i) (elt b i)))
                h
          )   )
          (AC (let ((h (make-array n)))
                (dotimes (j n) (setf (aref h j) (elt c j)))
                h
          )   )
          (AD 0)
      ...
      (flet
        ...
        (let ((elbar (make-array m :fill-pointer 0)) (elbar-fp 0)
              (not-elbar (make-array m :fill-pointer 0)) (not-elbar-fp 0))
          ...
          (dotimes (i m)
            ...
            (let ((hmax 0) (l nil))
              ...
              (if l
                ...
                (progn
                  (vector-push i not-elbar not-elbar-fp)
                  (pivot i l)
                )
                ; trivial row
                (if (zerop (aref AB i))
          ...
          (setq m not-elbar-fp) ; new number of rows = number of the ZO columns
        )
        ...
        (let ((Zeile_save (copy-seq Zeile)))
          (flet
            ((SuchePivotZeile (AWZM l AWZM-fp)
            ...
            (let ((PZM (make-array m :fill-pointer 0)) (PZM-fp 0)
                  (p nil))
              ...
              (loop ; fill column AB with positive numbers
                ...
                (let ((NZM (make-array m :fill-pointer 0)) (NZM-fp 0))
                  ; NZM = set of the i with AB[i]<0
                  ; recalculate PZM and NZM:
                  (let ((old-PZM-count PZM-fp)) ; old cardinality of PZM
                    (setf PZM-fp 0)
                    (dotimes (i m)
                      (if (>= (aref AB i) 0)
                        (vector-push i PZM PZM-fp)
                        (vector-push i NZM NZM-fp)
                    ) )
                    ; delete the additional columns if PZM really grew
                    ; and the degeneracy perhaps disappeared:
                    (when (> PZM-fp old-PZM-count)
                      (setq Extend nil)
                      (setq p nil)
                    )
                    ; otherwise PZM remained unchanged, and AB[p]<0 still holds.
                  )
                  (when (eql NZM-fp 0) (return)) ; every AB[i]>=0 ?
                  (if p
                    ; use the last p.
                  ...
                  (let ((hmin 0) (l nil))
                    ...
                    ; build AWZM:
                    (let ((AWZM (make-array m :fill-pointer 0)) (AWZM-fp 0))
                      (dotimes (ih PZM-fp)
                        (let ((i (aref PZM ih)))
                          (when (> (aref AA i l) 0) (vector-push i AWZM AWZM-fp))
                      ) )
                      (let ((k (SuchePivotZeile AWZM l AWZM-fp)))
                        (if (null k)
