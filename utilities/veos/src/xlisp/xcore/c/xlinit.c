...
#define MODULE_XLINIT_C_GLOBALS
#include "../../xmodules.h"
#undef MODULE_XLINIT_C_GLOBALS

/* xlinit - xlisp initialization routine */
xlinit()
{
    /* initialize xlisp (must be in this order) */
    xlfinit();  /* initialize the function table */	/* Voodoo */
...
/* Include hybrid-class functions: *//* JSP */
#define MODULE_XLINIT_C_XLINIT
#include "../../xmodules.h"
#undef MODULE_XLINIT_C_XLINIT
}
...
LOCAL initwks()
{
    ...
    /* To prevent obscure gc() crashes during initialization, it is   */
    /* important to establish s_gcflag and s_gchook early, and to set */
    /* them to NIL immediately.  This is Tom Almy's improvement on my */
    /* earlier kludge: (91Jan18jsp)                                   */
    s_gcflag = xlenter("*GC-FLAG*");    setvalue(s_gcflag,NIL);
    s_gchook = xlenter("*GC-HOOK*");    setvalue(s_gchook,NIL);
...
xlsymbols()
{
    ...
    s_send	= xlenter("SEND"); /*91Jun15jsp*/
...
/* Include hybrid-class functions: *//* JSP */
#define MODULE_XLINIT_C_XLSYMBOLS
#include "../../xmodules.h"
#undef MODULE_XLINIT_C_XLSYMBOLS
}
