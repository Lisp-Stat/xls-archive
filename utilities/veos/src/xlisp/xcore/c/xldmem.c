...
#define MODULE_XLDMEM_C_GLOBALS
#include "../../xmodules.h"
#undef MODULE_XLDMEM_C_GLOBALS
...
/* Include hybrid-class functions: *//* JSP */
#define MODULE_XLDMEM_C_GC
#include "../../xmodules.h"
#undef MODULE_XLDMEM_C_GC

    /* mark the obarray, the argument list and the current environment */
    if (obarray)   mark(obarray);
    if (xlenv)     mark(xlenv  );
    if (xlfenv)    mark(xlfenv );
    if (xldenv)    mark(xldenv );
    ...
    if (nfree < (long)anodes)   addseg();	 /*91Jan17jsp*/

    /* Call the *gc-hook* if necessary */
    if (s_gchook                   != NIL   &&
        (fun = getvalue(s_gchook)) != NIL
    ) {

	/* Rebind the hook fn to NIL: *//*91Jan17jsp*/
	LVAL olddenv = xldenv;	        /*91Jan17jsp*/
	xldbind(s_gchook,NIL);	        /*91Jan17jsp*/
	...
	/* Restore *GC-HOOK* binding: *//*91Jan17jsp*/
	xlunbind(olddenv);		/*91Jan17jsp*/
    }
  ...
  LVAL ptr;
{
	...
	while (!(this->n_flags & MARK)) {
		...
		switch (type) {
/* Include hybrid-class functions: *//* JSP */
#define MODULE_XLDMEM_C_MARK
#include "../../xmodules.h"
#undef MODULE_XLDMEM_C_MARK
		case SYMBOL:
		case OBJECT:
		case VECTOR:
		case CLOSURE:
...
LOCAL sweep()
{
	...
	for (n = seg->sg_size; --n >= 0; ++p) {
		...
		case STREAM:
			...
			break;
/* Include hybrid-class functions: *//* JSP */
#define MODULE_XLDMEM_C_SWEEP
#include "../../xmodules.h"
#undef MODULE_XLDMEM_C_SWEEP
		case SYMBOL:
		case OBJECT:
		case VECTOR:
		case CLOSURE:
...
xlminit()
{
    ...
    /* Since newvector etc depend on NULL==0, */  /* JSP */
    /* do a quick check:                      */  /* JSP */
    {						  /* JSP */
        LVAL*v = (LVAL*)calloc(sizeof(LVAL),1);   /* JSP */
        if (*v != NULL) xlfatal("NULL != 0");     /* JSP */
        free(v);				  /* JSP */
    }						  /* JSP */
...
/* Include hybrid-class functions: *//* JSP */
#define MODULE_XLDMEM_C_XLMINIT
#include "../../xmodules.h"
#undef MODULE_XLDMEM_C_XLMINIT
