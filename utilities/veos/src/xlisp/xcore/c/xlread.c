  ...
  char *fname; int vflag,pflag;
{
    ...
    /** let user choose path name **/   /* Voodoo */

    fullname[0] = '\0';					    	   

    /* find user path from load-path variable */
    if ((path = xlxgetvalue(xlenter("LOAD-PATH"))) != s_unbound && 
	stringp(path)) {		  			   
	strcpy(fullname,getstring(path)); 			   
  ...
  int ch;
{
    ...
    if (!vectorp(rtable) || ch < 0 || ch >= getsz(rtable))
	return (NIL);
