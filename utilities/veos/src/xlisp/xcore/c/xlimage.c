...
#define MODULE_XLIMAGE_C_XLISAVE
#include "../../xmodules.h"
#undef MODULE_XLIMAGE_C_XLISAVE
  ...
  char *fname;
{
	...
	for (n = seg->sg_size; --n >= 0; ++p)
	    switch (ntype(p)) {
/* Include hybrid-class functions: *//* JSP */
#define MODULE_XLIMAGE_C_XLIRESTORE
#include "../../xmodules.h"
#undef MODULE_XLIMAGE_C_XLIRESTORE
	    case SYMBOL:
	    case OBJECT:
	    case VECTOR:
	    case CLOSURE:
...
LOCAL freeimage()
{
	...
	for (n = seg->sg_size; --n >= 0; ++p)
	    switch (ntype(p)) {
/* Include hybrid-class functions: *//* JSP */
#define MODULE_XLIMAGE_C_FREEIMAGE
#include "../../xmodules.h"
#undef MODULE_XLIMAGE_C_FREEIMAGE
	    case SYMBOL:
	    case OBJECT:
	    case VECTOR:
	    case CLOSURE:
