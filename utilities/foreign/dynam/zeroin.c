/* This is Zeroin, that classic program by Forsythe et. al. Since I
   need it quite often in xlisp-stat, here it is. 

   Converted from the Netlib C version to xlisp by 
...
{
  LVAL f, args;
  FLOTYPE a,b,c;			/* Abscissae, descr. see above	*/
  FLOTYPE fa;				/* f(a)				*/
  FLOTYPE fb;				/* f(b)				*/
  FLOTYPE fc;				/* f(c)				*/
  ...
  args = consa(cvflonum(a));
  fa = getflonum(xlapply(pushargs(f, args)));

  rplaca(args, cvflonum(b));
  fb = getflonum(xlapply(pushargs(f, args)));

  c = a; fc = fa;

  for(;;)		/* Main iteration loop	*/
  {
    FLOTYPE prev_step = b-a;		/* Distance from the last but one*/
					/* to the last approximation	*/
    FLOTYPE tol_act;			/* Actual tolerance		*/
    FLOTYPE p;      			/* Interpolation step is calcu- */
    FLOTYPE q;      			/* lated in the form p/q; divi- */
    ...
    tol_act = 2*EPSILON*fabs(b);
    new_step = (c-b)/2;

    if( fabs(new_step) <= tol_act || fb == (FLOTYPE)0 )
      {
	xlpop();
	return cons(cvflonum(fb),args);	/* Acceptable approx. is found	*/
      }
    /* Decide if the interpolation can be tried	*/
    if( fabs(prev_step) >= tol_act	/* If prev_step was large enough*/
	&& fabs(fa) > fabs(fb) )	/* and was in true direction,	*/
    {					/* Interpolatiom may be tried	*/
	register FLOTYPE t1,cb,t2;
	cb = c-b;
	...
	if( p>(FLOTYPE)0 )		/* p was calculated with the op-*/
