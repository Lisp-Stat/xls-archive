...
This is the same as kde.c, but I use lists instead of vectors here.
Use as follows:
(dyn-load "kde1.o")
(def z (normal-rand 1000))
(call-lfun "kde" z)
...
/*  xllastarg();  /* Complain if more args */
  ...
  case 'G':			/* Gaussian Kernel */
    for (nextx = xo_arg, nexty = yo_arg; consp(nextx); nextx = cdr(nextx), nexty = cdr(nexty)) {
      temp1 = value(nextx);
