...
;   starting from a file from Jan Deleew (?)
...
This is a minor remake, changing the name of the game from :gnu-plot 
to :to-gnuplot and adding some key-arguments to the main method
to allow calling it without user interaction.


MINI-USER'S MANUAL (you should have got a file gnulsp.tex with this file)
...
6) load the resulting file in gnuplot
   (the one named <title>.gnu where <title> is the title of your object.

If in Unix and using (send my-obj :gnuplot :run-gnuplot t) step 6 will
...
For your knowledge, if you send :gnuplot to an object named 'name',
...
;;;;;;;;;;;;;provide
;;;if problems with release 3 Comment out this line:
(provide "gnuplot")
