...
(defmeth logit-link :means (eta)
  (let ((exp-eta (exp eta)))
    (/ exp-eta (+ 1 exp-eta))))
(defmeth logit-link :derivs (p) (+ (/ (if-else (= 0 p)     ; RAS zero div
                                               .00001
                                               p))
                                   (/ (if-else (= 1 p)
                                               .99999
...
(defmeth glim-proto :compute ()
  (let* ((epsilon (send self :epsilon))
    ...
    (do ((count 1 (+ count 1))
        ...
        (if verbose 
            (format t "Iteration ~d: deviance = ~a~%" 
...
marked as aliased."
  (let ((coefs (coerce (send self :coef-estimates) 'list))
    ...
    (format t "~%")
    (if (send self :estimate-scale)
        (format t "Scale Estimate:        ~10g~%" (send self :scale))
        (format t "Scale taken as:        ~10g~%" (send self :scale)))
    (format t "Deviance:              ~10g~%" (send self :deviance))
...
(defmeth binomialreg-proto :fit-variances (mu)
  (let* ((n (send self :trials))
         (p (/ mu n))
         (v (* n p (- 1 p)))   )
    (if-else (= 0 v) .00001 v)))    ; RAS to avoid zero div
...
(defmeth binomialreg-proto :fit-deviances (mu)
  (flet ((log+ (x) (log (if-else (< 0 x) x 1)))) ; to prevent log of zero
    (let* ((tol 1e-50)
           ...
           (n-y (- n y))
           (n-mu (- n mu))
           (pw (send self :pweights))      ; RAS avoid div by zero
           (raw-dev (* 2 (+ (* y (log+ (/ y 
                                          (if-else (< mu tol) tol mu))))
                            (* n-y (log+ (/ n-y (if-else (< n-mu tol)
                                                         tol n-mu))))))))
      (if pw (* pw raw-dev) raw-dev))))
