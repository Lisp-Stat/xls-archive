...
original data x and transform-list tlist"
  (let ((z NIL)
	(nx (if (matrixp x) (column-list x) x)))
    (dotimes (i (length nx) 
		(if z (apply #'bind-columns (reverse z)) nil))
	     (setf z (append (construct-transformed-predictor 
			      (nth i nx) i tlist) z)))))
  ...
  (let ((z nil)
    ...
    (dotimes (i (length vb-select) (reverse z))
             (let ((xtf (nth (1+ i) tlist)))
               (if (nth i vb-select)
                   (setf index (+ index (if (equal xtf '(NULL)) 0	
                                            (length xtf))))
                   (progn
                    (setf z 
                          (cons 
...
"
Args: (elmts x)
Returns a sublist of x with elements indexed in elmts deleted.
elmts may be a single number if only one element of x is to be removed,
...
(defun add-var-tval (x y v w)
  (let* ((ax (bind-columns x v))
         (rm (reg-model ax y :print nil :weights w)))
    (/ (car (last (send rm :coef-estimates)))
       (car (last (send rm :coef-standard-errors))))))
                  
(defun num-elmts-list (l)
  (let ((total 0))
    (dolist (i l total)
	    (setf total (+ total (if (listp i) 
			     (num-elmts-list i)
			   (if (equal i 'NULL) 0 1)))))))

(defun var-to-add (indices x y w origx &key (p-value 0.05))
  (if indices
      (let* ((t-vals (mapcar #'(lambda (v) (add-var-tval x y v w)) 
                              (select origx indices)))
	     (dim (array-dimensions x))
	     (df (- (car dim) (cadr dim) 2)))
	    (if (> (max (abs t-vals)) 
		   (abs (t-quant (/ p-value 2) df)))
		(car (select indices (which (= (max (abs t-vals)) 
					       (abs t-vals)))))))))

(defun concat-str-list (l)
  (let ((z (car l)))
    (dolist (i (cdr l) z)
	    (setf z (format nil "~a, ~a" z i)))))


(defun indic-vars-sel (l v)
  (let ((n (length v))
	(res nil))
    (dotimes (i n (which (reverse res)))
	     (let ((vs (nth i v))
		   (ls (nth i l)))
	       (cond ((null vs) (push t res))
		     ((not (equal ls '(NULL)))
		      (setf res (append (repeat '(nil) (length ls)) res))))))))
