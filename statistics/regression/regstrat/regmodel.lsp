...
(defproto reg-model-proto
          '(tran-list vb-select history children ancestors terminal-node origx)
          '()
...
Returns a regression model object."
  (let ((x (cond 
    ...
    (if origx
	(send m :origx origx)
      (send m :origx (if (matrixp x) (column-list x) x)))
    (if (and tran-list (null origx))
	(send m :x (construct-transformed-design-matrix x tran-list))
      (send m :x (if (matrixp x) x (apply #'bind-columns x))))
    (if (and tran-list origx)
	(send m :x x))
    (send m :y y)
...
Returns an expression that will reconstruct the regression model."
  `(reg-model ',(send self :x)
	      ...
	      :ancestors ',(send self :ancestors)
	      :terminal-node ',(send self :terminal-node)))
...
are list pairs - first element being the action, the second the result"
  (if set (setf (slot-value 'history) (if (and (listp flist) (listp (car (reverse flist))))
					  (reverse flist)
		(cons flist (slot-value 'history)))))
...
(defmeth reg-model-proto :tran-predictors  
  (&key (diag-print t)  (p-value 0.05))
...
at interpretation point(s) mp"
  (decon-interpret (cdr (send self :coef-estimates)) 
                   (send self :tran-list)
                   mp
                   (mapcar (caar (send self :tran-list)) 
                           (send self :prediction mp))
                   (send self :vb-select)))
...
weight points - used by :restore-points"
  (let ((r (send self :externally-studentized-residuals))
	...
	(let* ((i (which (= w 0)))
	  ...
	  (setf (select r i) rr)))
    r))
(defmeth reg-model-proto :children (&optional flist)
  "Message args: (&optional flist)
With no argument returns the children list. FLIST is consed to the children."
  (if flist (push flist (slot-value 'children)))
  (slot-value 'children))

(defmeth reg-model-proto :ancestors (&optional flist)
  "Message args: (&optional flist)
With no argument returns the ancestors list. FLIST is consed to the ancestors."
  (if flist (push flist (slot-value 'ancestors)))
  (slot-value 'ancestors))

(defmeth reg-model-proto :terminal-node (&optional (flist nil set))
  "Message args: (&optional flist)
With no argument returns the terminal-node list. Terminal-node is set to FLIST."
  (if set (setf (slot-value 'terminal-node) flist))
  (slot-value 'terminal-node))

(defmeth reg-model-proto :origx (&optional (flist nil set))
  "Message args: (&optional flist)
With no argument returns the origx list. origx is set to FLIST."
  (if set (setf (slot-value 'origx) flist))
  (slot-value 'origx))

(defmeth reg-model-proto :fw-sel (&key (p-value 0.05))
  (let ((s "Forward Selection")
	(added nil)
	(pn (send self :predictor-names))
	(y (send self :y))
	(w (send self :weights))
	(origx (send self :origx))
	(changed nil))
    (loop (let* ((x (send self :x))
		 (tl (send self :tran-list))
		 (indices (which (mapcar #'(lambda (l) 
                                     (equal l '(NULL))) (cdr tl))))
		 (ins (var-to-add indices x y w origx :p-value p-value)))
	    (if ins
		(let* ((ip (num-elmts-list (select tl (iseq 1 ins))))
		       (nx (insert-column x (nth ins origx) ip)))
		  (setf (select tl (1+ ins)) '(IDENTITY))
		  (send self :x nx)
		  (push ins added)
		  (setf changed t))
	      (progn
		(if (car (send self :history))
		    (if changed
			(let* ((a (concat-str-list (select pn added)))
			       (b (format nil "~a added" a)))
			  (send self :history (list s b)))
		      (send self :history (list s "No Change"))))
		(return changed)))))))

(defmeth reg-model-proto :predun (x)
"Args: (x) Returns untransformed predicted value with se at x
for reg-model"
  (let* ((lx (if (listp (car x)) x (list x)))
         (ests (send self :coef-estimates))
         (ax (tranx lx (send self :tran-list)))
         (ba (cons 0 (1+ (send self :basis))))
         (pp (mapcar #'(lambda (a) (sum (* (select a ba) ests))) ax))
         (sigh (send self :sigma-hat))
         (xtxinv  (send self :xtxinv))
         (ytf (caar (send self :tran-list)))
         (se (mapcar #'(lambda (nax) (* sigh
             (sqrt (sum (* (%* nax xtxinv) nax))))) ax)))
    (list ytf (car pp) (car se))))
