...
Creates a new instance of the kde-proto object."
  ...
  (let* ((d (combine (send self :data))) ;Made this change for the RM scheme.
	 (fv (fivnum d))
	 (a (min (standard-deviation d) 
		 (/ (- (select fv 3) (select fv 1)) 1.34))))
    (setf (slot-value 'kernel-window-width)
	  (* 0.9 a (^ (send self :n) -.2)))
    (setf (slot-value 'x-vals) 
	  (rseq (select fv 0) (select fv 4) (send self :kernel-points))))
  ...
  (call-next-method 2 :go-away go-away :size *kde-plot-size*)
...
Accessor method for the slot 'kernel-points."
  (if num
	...
	(setf (slot-value 'x-vals) (rseq (min d) (max d) num))
...
Lets the user change Kernel bin width using a slider"
  (let* ((ti (send text-item-proto :new "Kernel Window Width"))
