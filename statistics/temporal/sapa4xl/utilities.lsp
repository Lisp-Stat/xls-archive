...
;-------------------------------------------------------------------------------
#+XLISP
(defmacro multf (x &optional (delta 1)) "Like <incf>" `(setf ,x (* ,x ,delta)))
#-XLISP
(define-modify-macro multf (&optional (delta 1)) * "Like <incf>")

#+XLISP
(defmacro divf (x &optional (delta 1)) "Like <incf>" `(setf ,x (/ ,x ,delta)))
#-XLISP
...
(defun circular-shift-sequence
       (a-seq
        &key
        (result
         (make-array (length a-seq) :initial-element 0)))
  "given
   [1] a-seq (required)
       ==> a sequence
